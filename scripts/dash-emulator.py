...
def create_parser():
    ...
    arg_parser.add_argument("--plot", required=False,
                            default=None, type=str, help="The folder to save plots")
    arg_parser.add_argument("--dump-results", required=False,
                            default=None, type=str, help="Dump the results")
    arg_parser.add_argument("--env", required=False,
                            default=None, type=str, help="Environment to use")
    arg_parser.add_argument("--abr", required=False, default='bandwidth-based',
                            type=str, help="Adaptation algorithm to use")
    arg_parser.add_argument("-y", required=False, default=False, action='store_true',
                            help="Automatically overwrite output folder")
    arg_parser.add_argument("--num", required=False, default=1,
                            type=int, help="Number of experiment repetition")
    arg_parser.add_argument(PLAYER_TARGET, type=str,
                            help="Target MPD file link")
    return arg_parser
    ...
    async def main():
        for i in range(args["num"]):
            player, analyzer = build_dash_player_over_quic(
                player_config, downloader_config, beta=args["beta"], plot_output=args["plot"], dump_results=args['dump_results'], abr=args['abr'])
